...
summary.bootnet <- function(
  ...
  tab <- x$bootTable %>% 
    ...
    dplyr::left_join(x$sampleTable %>% dplyr::select(type,id, node1,node2,sample=value), by=c("id","type","node1","node2")) %>%
    dplyr::select(type, id, node1, node2, sample, mean, var, q1, q2.5, q5, q25, q50, q75, q95, q97.5, q99)
